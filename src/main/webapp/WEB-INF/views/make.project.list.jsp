		...
		<c:if test="${count > 0}">
			...
			<ul class="pagination">
				<c:if test="${numPageGroup > 1}">
					<li><a
						href="./make_project_list?project_no=${project_no}&pageNum=${(numPageGroup-2)*pageGroupSize+1}">prev</a></li>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<li><a href="./make_project_list?project_no=${project_no}&pageNum=${i}"> <c:if
								test="${currentPage == i}">
							</c:if> ${i}
					</a></li>
				</c:forEach>
				<c:if test="${numPageGroup < pageGroupCount}">
					<li><a href="./make_project_list?project_no=${project_no}&pageNum=${numPageGroup*pageGroupSize+1}">next</a></li>
				</c:if>
			</ul>
		</c:if>

		<div>
			<button type="button" onclick="form_save('#form');">적용</button>
		</div>
