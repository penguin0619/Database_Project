...
<div class="navbar navbar-inverse navbar-fixed-top">
  ...
  <thead>
    ...
    <tr onclick="location.href='./history_project_view?no=${item.project_request_no}'">
		...
		<c:if test="${count > 0}">
			...
			<ul class="pagination">
				<c:if test="${numPageGroup > 1}">
					<li><a
						href="./history_project_list?pageNum=${(numPageGroup-2)*pageGroupSize+1}">prev</a></li>
				</c:if>
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<li><a href="./history_project_list?pageNum=${i}"> <c:if
								test="${currentPage == i}">
							</c:if> ${i}
					</a></li>
				</c:forEach>
				<c:if test="${numPageGroup < pageGroupCount}">
					<li><a href="./history_project_list?pageNum=${numPageGroup*pageGroupSize+1}">next</a></li>
				</c:if>
			</ul>
		</c:if>
